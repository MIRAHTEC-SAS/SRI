...
/******************************* Changement de Statut Incident ***************************/

if (isset($_POST['changeStatutIncident'])) {
    ...
    // Maj Historique statut Intervention
    $sql = mysqli_query($con, "INSERT INTO `historique_statuts_incident` (`numero_incident`, `statut`, `date_statut`, `auteur`) 
        VALUES ('$numero_incident', '$statut', '$date_saisie', '$auteur')");

    // Mettre a jour le statut du signalement
    $sql = mysqli_query($con, "UPDATE signalements SET statut='$statut' WHERE numero_incident='$numero_incident'");
    // echo 'done';die;
    $_SESSION['errorMsg'] = false;
    $_SESSION['successMsg'] = true;
    $_SESSION['message'] = "Statut de l'incident modifié avec succès!";
    header("Location: ../signalements");
}
