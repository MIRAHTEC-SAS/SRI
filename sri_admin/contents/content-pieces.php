...
$getPieces = mysqli_query($con, "SELECT * FROM `pieces` INNER JOIN etages on etages.code_etage=pieces.code_etage INNER JOIN batiments ON batiments.code_batiment=etages.code_batiment order by pieces.id desc");
			...
			<?php if (!empty($getPieces)) while ($row = mysqli_fetch_array($getPieces)) {  ?>
				<tr>
					...
					<td style="text-align:center">
						<a href="pieces.php?edit=<?php echo $row['code_piece']; ?>" class="text-info me-10" data-bs-toggle="tooltip" data-bs-original-title="Edit">
							<i class="fa fa-edit" style="font-size:16px;color:orange"></i>
						</a>
						<a href="pieces.php?delete=<?php echo $row['code_piece']; ?>" class="text-danger" data-bs-original-title="Delete" data-bs-toggle="tooltip">
