...
if (isset($_SESSION['User']) && isset($_SESSION['UserPass']) && $_SESSION['role'] == 'Administrateur' || $_SESSION['role'] == 'Responsable'  || $_SESSION['role'] == 'Gestionnaire' || $_SESSION['role'] == 'Intervenant') {
	...
	switch ($profil) {
		case 'Administrateur':
            ...
            signalements.statut as statut_incident, 
			...
			INNER JOIN type_incidents ON type_incidents.code_incident=interventions.code_incident WHERE interventions.statut='annulee'");
			break;
		case 'Responsable':
		...
		case 'Gestionnaire':
			...
			INNER JOIN signalements ON signalements.numero_incident=interventions.numero_incident
			INNER JOIN type_incidents ON type_incidents.code_incident=interventions.code_incident 
			WHERE interventions.statut='annulee' AND services.code_service='$code_service'");
			break;
		case 'Intervenant':
			$getInterventionsAnnulees = mysqli_query($con, "SELECT 
			interventions.code_intervention,
			...
			FROM `interventions`
			INNER JOIN services ON services.code_service=interventions.service
			INNER JOIN type_incidents ON type_incidents.code_incident=interventions.code_incident WHERE interventions.statut='annulee' AND intervenant='$matricule'");
			break;
	}
								...
								<div class="d-inline-block align-items-center">
									<nav>
										<ol class="breadcrumb">
											...
											<li class="breadcrumb-item active" aria-current="page">Annulées</li>
										</ol>
									</nav>
								</div>
							</div>
							...
							<div class="col-12">
								<div class="box">
									<div class="box-header with-border">
										<h4 class="box-title">Liste des interventions annulées</h4>
									</div>
									<div class="box-body" style="background-color:#FBFBFB">
										<?php include('contents/content-interventions_annulees.php'); ?>
									</div>
									<!-- /.box-body -->
