...
if (isset($_SESSION['User']) && isset($_SESSION['UserPass']) && $_SESSION['role'] == 'Administrateur' || $_SESSION['role'] == 'Responsable' || $_SESSION['role'] == 'Gestionnaire') {
	...
	switch ($profil) {
		case 'Administrateur':
			...
			WHERE signalements.statut='termine'");
			break;
		case 'Responsable':
			...
			INNER JOIN code_priorite ON code_priorite.code=signalements.code_priorite 
			INNER JOIN interventions ON interventions.numero_incident=signalements.numero_incident
			WHERE signalements.statut='termine' AND signalements.code_incident IN ('$t_i_0','$t_i_1','$t_i_2','$t_i_3','$t_i_4')");
			break;
		case 'Gestionnaire':
			$getSignalements = mysqli_query($con, "SELECT 
			signalements.date_reception,
			...
			INNER JOIN code_priorite ON code_priorite.code=signalements.code_priorite 
			INNER JOIN interventions ON interventions.numero_incident=signalements.numero_incident
			WHERE signalements.statut='termine' AND services.code_service='$code_service'");
			break;
	}
							...
							<div class="col-12">
								<div class="box">
									<div class="box-header with-border">
										<h4 class="box-title">Liste des incidents traités</h4>
									</div>
									<div class="box-body" style="background-color:#FBFBFB">
										<?php include('contents/content-signalements-termines.php'); ?>
									</div>
									<!-- /.box-body -->
