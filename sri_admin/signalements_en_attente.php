...
if (isset($_SESSION['User']) && isset($_SESSION['UserPass']) && $_SESSION['role'] == 'Administrateur' || $_SESSION['role'] == 'Responsable' || $_SESSION['role'] == 'Gestionnaire') {
	...
	switch ($profil) {
		case 'Administrateur':
			...
			FROM `signalements` INNER JOIN services on services.code_service=signalements.code_service INNER JOIN type_incidents ON type_incidents.code_incident=signalements.code_incident INNER JOIN code_priorite ON code_priorite.code=signalements.code_priorite where signalements.statut='en attente' ORDER BY signalements.id DESC");
			break;
		case 'Responsable':
		...
		case 'Gestionnaire':
			...
			code_priorite.couleur_priorite
			FROM `signalements` INNER JOIN services on services.code_service=signalements.code_service INNER JOIN type_incidents ON type_incidents.code_incident=signalements.code_incident INNER JOIN code_priorite ON code_priorite.code=signalements.code_priorite 
			WHERE signalements.statut='en attente' AND services.code_service='$code_service' ORDER BY signalements.numero_incident DESC");
			break;
	}
?>
	<?php
							...
							<div class="col-12">
								<div class="box">
									<div class="box-header with-border">
										<h4 class="box-title">Liste des incidents en attente de traitement</h4>
									</div>
									<div class="box-body" style="background-color:#FBFBFB">
										<?php include('contents/content-signalements-en-attente.php'); ?>
									</div>
									<!-- /.box-body -->
