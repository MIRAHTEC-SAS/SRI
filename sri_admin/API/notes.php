...
for ($i = 0; $i < count($mat); $i++) 
{
    ...
    $sql= mysqli_query($con, "INSERT INTO fc_notees_enregistrees (codeFc,codeDirection,codeMinistere,matricule, hierarchie,nb_part,decade,note,points,date_saisie)
        VALUES ('$codeFc', '$codeDirection', '$codeMinistere', '$mat[$i]', '$hierarchies[$i]', '$parts[$i]', '$decades[$i]', '$notes[$i]', '$points', '$dateDuJour')");
    
    $sql= mysqli_query($con, "INSERT INTO fc_notees_enregistrees_ref (codeFc,codeDirection,codeMinistere,matricule, hierarchie,nb_part,decade,note,points,date_saisie)
    VALUES ('$codeFc', '$codeDirection', '$codeMinistere', '$mat[$i]', '$hierarchies[$i]', '$parts[$i]', '$decades[$i]', '$notes[$i]', '$points', '$dateDuJour')");
...
$sql = $con->query("INSERT INTO directions_notees (codeFc,codeDirection,codeMinistere,nbAgent,total_points,auteur,date_saisie) 
VALUES ('$codeFc','$codeDirection','$codeMinistere','$nbAgent','$pointsDirection','$auteur','$dateDuJour')");

$sql = $con->query("DELETE FROM fc_en_cours_notation where codeFc='$codeFc' and codeDirection='$codeDirection'");

// Mise a jour du step
$step="Notes enregistrees";
...
$_SESSION['message'] ="Note enregistrée avec succès ! </br> Veuillez valider les notes pour une prise en compte.";
