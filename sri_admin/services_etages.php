                			...
                			<form action="services/parametrages.php" method="POST">
								<div class="row">
									...
									<label class="col-md-12 form-label">Service</label>
										...
										<?php } ?>
											<?php if ($update>0) {?>
												...
												<?php while ($row = mysqli_fetch_array($getServices2)) { $code=$row['code_service'];
									...
									<div class="col-12" >
										...
										<?php if ($update==1) {?> <button type="submit" name="modifierServiceIntervenant" class="btn btn-warning form-control" style="color:black;height:50%"><i class="fa fa-edit" ></i> Modifier affectation</button><?php } ?>
										<?php if ($update==2) {?> <button type="submit" name="supprimerServiceIntervenant" class="btn btn-danger form-control" style="color:white;height:50%"><i class="fa fa-trash" ></i> Supprimer</button><?php } ?>
									</div>
								</div>
							</form>
						</div>
...
<?php } else { header('Location: ../sri_admin');}  ?>