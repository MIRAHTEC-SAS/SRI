...
$client = new Client($account_sid, $auth_token);
 ...
 if (isset($_POST['notifierCandidats'])) {
    ...
    $verifIntegrite = mysqli_query($con, "SELECT * FROM concours_notifies WHERE codeConcours='$codeConcours'");

    if (mysqli_num_rows($verifIntegrite) > 0)
    {
      $sql = $con->query("UPDATE concours_notifies SET date_notification='$dateDuJour' WHERE code_concours='$codeConcours'");
    }
    else
    {
      $sql = $con->query("INSERT INTO concours_notifies (code_concours, date_notification) VALUES ('$codeConcours', '$dateDuJour')");
    }
