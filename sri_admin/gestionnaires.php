...
if (isset($_SESSION['User']) && isset($_SESSION['UserPass']) && $_SESSION['role'] == 'Administrateur' || $_SESSION['role'] == 'Responsable') {
	...
	$getInfosGestionnaires = mysqli_query($con, "SELECT * FROM `gestionnaires` order by id desc");
...
?>
	...
	if (isset($_GET['edit'])) {
		...
		$matricule_gestionnaire = $_GET['edit'];

		$getInfosGestionnaire = mysqli_query($con, "SELECT * FROM gestionnaires where matricule_gestionnaire='$matricule_gestionnaire'");
		while ($row = mysqli_fetch_array($getInfosGestionnaire)) {
			$prenom = $row['prenom'];
			$nom = $row['nom'];
	...
	if (isset($_GET['delete'])) {
		...
		$matricule_gestionnaire = $_GET['delete'];
											...
											<form action="controllers/referentielController.php" method="POST">
												<div class="row">
													...
													<div class="col-3">
														...
														<input type="hidden" name="matricule_gestionnaire" value="<?php echo $matricule_gestionnaire; ?>">
													</div>
												</div>
												</br>
												...
												<div class="row">
													<div class="container-fluid">
														...
														<?php if ($update == 1) { ?> <button type="submit" name="modifierGestionnaire" class="btn btn-warning form-control" style="color:black;height:90%"><i class="fa fa-edit"></i> Modifier</button><?php } ?>
														<?php if ($update == 2) { ?> <button type="submit" name="supprimerGestionnaire" class="btn btn-danger form-control" style="color:white;height:90%"><i class="fa fa-trash"></i> Supprimer</button><?php } ?>
													</div>
												</div>
										</div>
										</form>
