...
if (isset($_SESSION['User']) && isset($_SESSION['UserPass']) && $_SESSION['role'] == 'Administrateur' || $_SESSION['role'] == 'Responsable'  || $_SESSION['role'] == 'Intervenant' || $_SESSION['role'] == 'Gestionnaire') {
	...
	switch ($profil) {
		...
		case 'Responsable':
			...
			INNER JOIN services ON services.code_service=interventions.service
			INNER JOIN type_incidents ON type_incidents.code_incident=interventions.code_incident 
			WHERE interventions.statut='terminee' AND interventions.code_incident IN ('$t_i_1','$t_i_2','$t_i_3','$t_i_4','$t_i_5')");
			break;
		case 'Gestionnaire':
			$getInterventionsTerminees = mysqli_query($con, "SELECT 
			interventions.code_intervention,
			...
			INNER JOIN services ON services.code_service=interventions.service
			INNER JOIN type_incidents ON type_incidents.code_incident=interventions.code_incident 
			WHERE interventions.statut='terminee' AND services.code_service='$code_service'");
			break;
		case 'Intervenant':
			$getInterventionsTerminees = mysqli_query($con, "SELECT 
			interventions.code_intervention,
			...
			WHERE interventions.statut='terminee' AND intervenants_interne.matricule_intervenant='$matricule'");
			break;
	}
								...
								<div class="d-inline-block align-items-center">
									<nav>
										<ol class="breadcrumb">
											...
											<li class="breadcrumb-item active" aria-current="page">Terminées</li>
										</ol>
									</nav>
								</div>
							</div>
							...
							<div class="col-12">
								<div class="box">
									<div class="box-header with-border">
										<h4 class="box-title">Liste des interventions terminées</h4>
									</div>
									<div class="box-body" style="background-color:#FBFBFB">

										<?php
										if ($profil == 'Intervenant') {
											include('contents/content-interventions_terminee_intervenant.php');
										} else {
											include('contents/content-interventions_terminees.php');
										}
										?>
