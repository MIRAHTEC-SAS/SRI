...
<script>
    const vue = new Vue({
        data: {
            ...
            urlPOSTAgent: "API/api_pgav.php?action=postAgent",
        ...
        computed: {
            directionsGeneralesParMinistere() {
                return this.directionsGenerales.filter(dg => dg.codeMinistere === this.newAgent.codeMinistere);
            },
            directionsParDg() {
                return this.directions.filter(direction => direction.codeDirectionGenerale === this.newAgent.codeDirectionGenerale);
            },
            corpsParSecteur() {
                return this.corps.filter(metier => metier.codeSecteur === this.newAgent.codeSecteur);
            },
            agencesParBanque() {
                return this.agences.filter(agence => agence.codeBanque === this.newAgent.codeBanque);
            },
        ...
        methods: {
            ...
            addAgent() {
                ...
                var formData = this.toFormData(this.newAgent);

                //On lance la requete GET (Create)
                axios.post(this.urlPOSTAgent, formData)
                    .then((resultat) => resultat.data)
                    .then((resultat) => {
                        this.newAgent = {
                        }
