...
if (isset($_POST['ajouterMinistere'])) {
    ...
    $sigle=$_POST['sigle'];
        ...
        if (mysqli_num_rows($verifIntegrite) > 0)
        {
            ...
            header("Location: ../ministeres.php");
        }
        else
        {
...
if (isset($_POST['ajouterResponsable'])) {
    ...
    while ($row = mysqli_fetch_array($getLastMatricule)) { 
        ...
        if (mysqli_num_rows($verifIntegrite) > 0)
        {
            ...
            header("Location: ../responsables_dage.php");
        }
        else
        {
...
if (isset($_POST['ajouterIntervenant'])) {
    ...
    while ($row = mysqli_fetch_array($getLastMatricule)) { 
        ...
        if (mysqli_num_rows($verifIntegrite) > 0)
        {
            ...
            header("Location: ../intervenants_dage.php");
        }
        else
        {
...
if (isset($_POST['ajouterTypeIncident'])) {
    ...
    // echo $couleur.'</br>';die;
        ...
        if (mysqli_num_rows($verifIntegrite) > 0)
        {
            ...
            header("Location: ../types_incident.php");
        }
        else
        {
...
if (isset($_POST['ajouterServiceIntervenant'])) {
    ...
    // $matricule_service=$_POST['matricule_service'];
        ...
        if (mysqli_num_rows($verifIntegrite) > 0)
        {
            ...
            $_SESSION['successMsg']=false;
            $_SESSION['message'] ="Ce service existe deja ! ";
            header("Location: ../services_intervenant.php");
        }
        else
        {
...
if (isset($_POST['ajouterUser'])) {
    ...
    switch ($code_role) 
    {
        ...
        case 6 :
            // Responsables....
            $matricule=$_POST['matricule'];
            $getInfos = mysqli_query($con, "SELECT * FROM intervenants_interne WHERE matricule_intervenant='$matricule'");
...
// Mot de pass oublié
if(isset($_POST['reinitialiserPassword'])) {

    $email = $_POST['email'];
    $pass_tmp='5d0cf713a106628663bb8c552f31d4491fc9f38a0878626b5af4b0d91a914291';
    $tempoPass='dtai@2023';

    $link_app='https://sri.minfinances.sn/sri_admin/';
    $getInfosUser = mysqli_query($con, "SELECT * FROM users WHERE email='$email'");
    while ($row = mysqli_fetch_array($getInfosUser)) { 
        $utilisateur=$row['prenom'].' '.$row['nom'];
    }
    ...
    $sql=mysqli_query($con, "UPDATE `users` SET password='$pass_tmp' WHERE email='$email'"); 

    // echo 'done !';die;

    if($sql) 
    {
        include ("mail_reinit_password.php");

        $_SESSION['errorMsg']=false;
        $_SESSION['successMsg']=true;
        $_SESSION['message'] = "Votre mot de passe est reinitialise avec succès !</br> Nous venons de vous envoyer le mot de passe temporaire par email."; 
        header ('Location: ../');  
    }
    else
    {
        $_SESSION['errorMsg']=true;
        $_SESSION['successMsg']=false;
        $_SESSION['message'] = "L'adresse mail saisi n'est pas connu du systeme !"; 
        header ('Location: ../');
    }
